...
class UnifiedTransformer(Model):
    ...
    def __init__(self, args, place):
        ...
        self.param_share = args.get("param_share", "normal")
    ...
    def _gen_input(self,
        ...
        if self.use_turn:
            turn_emb_out = layers.embedding(
                ...
                dtype=self.dtype,
                param_attr=paddle.ParamAttr(
                    name=name + self.turn_emb_name, initializer=self.param_initializer))
            emb_out = emb_out + turn_emb_out

        # concat auxiliary memory embeddings
        if aux_emb is not None:
            emb_out = paddle.concat(x=[aux_emb, emb_out], axis=1)

        if self.emb_mapping_in:
            emb_out = paddle.statistics.nn.fc(
                x=emb_out,
                num_flatten_dims=2,
        ...
        if isinstance(input_mask, (tuple, list)):
            attn_bias = (
                paddle.unsqueeze(
                    x=paddle.scale(x=input_mask, scale=1e4, bias=-1.0, bias_after_scale=False), axis=1)
                for mask in input_mask
            )
        else:
            attn_bias = paddle.unsqueeze(
                x=paddle.scale(x=input_mask, scale=1e4, bias=-1.0, bias_after_scale=False), axis=1)
    ...
    def _get_pooled_output(self, enc_out, idx=None, name="pooled"):
        ...
        if idx is None:
            ...
            feat = paddle.gather(x=enc_out, index=idx)
        elif len(idx.shape) == 2 and idx.shape[1] == 2:
            feat = paddle.gather_nd(x=enc_out, index=idx)
        else:
            raise ValueError(f"Invalid indices shape {idx.shape} is used")

        pooled_out = paddle.static.nn.fc(
            x=feat,
            size=self.hidden_size,
            activation="tanh",
            weight_attr=paddle.ParamAttr(name=f"{name}_fc.w_0", initializer=self.param_initializer),
    ...
    def _get_classifier_output(self, pooled_out, num_classes=2, name="cls"):
        ...
        """
        cls_logits = paddle.static.fn.fc(
            x=pooled_out,
            size=num_classes,
            weight_attr=paddle.ParamAttr(name=f"{name}_fc.w_0", initializer=self.param_initializer),
            bias_attr=f"{name}_fc.b_0")
    ...
    def _calc_logits(self, enc_out, tgt_idx=None, name=""):
        ...
        if tgt_idx is None:
            ...
            seq_feat = paddle.gather_nd(x=enc_out, index=tgt_idx)
        else:
            raise ValueError(f"Invalid indices shape {tgt_idx.shape} is used")

        seq_trans_feat = paddle.static.nn.fc(
            x=seq_feat,
            size=self.emb_size,
        ...
        if self.weight_sharing:
            logits = paddle.matmul(
                ...
                transpose_y=True)
            if self.cls_bias:
                logits += paddle.create_parameter(
                    shape=[self.vocab_size],
                    dtype=self.dtype,
                    attr=paddle.ParamAttr(name="mask_lm_out_fc.b_0"),
                    is_bias=True)
        else:
    ...
    def _get_feed_dict(self, is_infer=False):
        ...
        if is_infer:
            ...
            feed_dict["init_score"] = paddle.static.data(name="init_score", shape=[-1, 1], dtype="float32", lod_level=1)
            feed_dict["parent_idx"] = paddle.static.data(name="parent_idx", shape=[-1], dtype="int64")
    ...
    def forward(self, inputs, is_infer=False):
        ...
        outputs = self._generation_network(
    ...
    def get_metrics(self, inputs, outputs):
        ...
        tgt_lm_loss = paddle.nn.functional.softmax_with_cross_entropy(
    ...
    def _initialize_state(self, inputs, step_idx):
        state = {}
        state["tgt_ids"] = paddle.tensor.array_write(paddle.reshape(inputs["tgt_ids"], [-1, 1]), step_idx)
        state["tgt_pos"] = paddle.tensor.array_write(paddle.reshape(inputs["tgt_pos"], [-1, 1, 1]), step_idx)
        state["scores"] = paddle.tensor.array_write(inputs["init_score"], step_idx)
        state["tgt_generation_mask"] = paddle.tensor.array_write(inputs["tgt_generation_mask"], step_idx)
    ...
    def _prepare_timestep_input(self, state, step_idx):
        ...
        # token ids
        pre_ids = paddle.tensor.array_read(array=state["tgt_ids"], i=step_idx)
        model_input["token_ids"] = paddle.unsqueeze(x=pre_ids, axis=1)

        # position ids
        pre_pos = paddle.tensor.array_read(array=state["tgt_pos"], i=step_idx)
        model_input["pos_ids"] = paddle.gather(pre_pos, state["parent_idx"])

        pre_scores = paddle.tensor.array_read(array=state["scores"], i=step_idx)

        # generation_mask
        tgt_generation_mask = paddle.tensor.array_read(state["tgt_generation_mask"], i=step_idx)
        append_mask = layers.fill_constant_batch_size_like(pre_ids, [-1, 1, 1], "float32", 1.0)
        tgt_generation_mask = paddle.concat(x=[tgt_generation_mask, append_mask], axis=2)

        model_input["generation_mask"] = pre_mask = paddle.gather(x=tgt_generation_mask, index=state["parent_idx"])
        ...
        if self.use_turn:
            model_input["turn_ids"] = layers.fill_constant_batch_size_like(pre_mask, [-1, 1, 1], "int64", 0)
    ...
    def _update_state(self,
                      ...
                      parent_idx,
                      step_idx):
        paddle.tensor.array_write(selected_ids, i=step_idx, array=state["tgt_ids"])
        paddle.tensor.array_write(selected_scores, i=step_idx, array=state["scores"])
        paddle.tensor.array_write(model_input["generation_mask"], i=step_idx, array=state["tgt_generation_mask"])
        paddle.tensor.array_write(model_input["pos_ids"] + 1, i=step_idx, array=state["tgt_pos"])
        paddle.assign(parent_idx, state["parent_idx"])
        return state
