...
def setup_args():
    ...
    parser.add_argument("--use_profiler", type=str2bool, default=False,
                        help="Whether use profiler for performance analysis.")
...
def train(args):
    ...
    if args.is_distributed:
        ...
        dev_count = paddle.framework.core.get_cuda_device_count()
        ...
        trainers_num = 1
        trainer_id = 0
    place = paddle.framework.CUDAPlace(gpu_id)
    ...
    for step, data in enumerate(train_generator(), args.start_step + 1):
        if args.use_profiler:
            if step == 10:
                ...
                print("Stop profiler")
                profiler.stop_profiler("total", os.path.join(args.save_path, "profiler"))
