    ...
    def forward(self, sparse_inputs, dense_inputs, show_click=None):
        ...
        for s_input in sparse_inputs:
            ...
            emb = paddle.fluid.contrib.sparse_embedding(
                    input=s_input,
                    size=[self.sparse_feature_number, self.sparse_feature_dim + 2],
                    param_attr=paddle.ParamAttr(name="embedding"))
            emb = paddle.fluid.layers.continuous_value_model(emb, show_click, False)
...
class WideDeepModel:
    ...
    def net(self, is_train=True):
        ...
        cast_label = paddle.cast(label_input, dtype='float32')
        ones = paddle.fluid.layers.fill_constant_batch_size_like(input=label_input, shape=[-1, 1], dtype="float32", value=1)
        show_click = paddle.cast(paddle.concat([ones, cast_label], axis=1), dtype='float32')
        show_click.stop_gradient = True
        pred = wide_deep_model.forward(sparse_inputs, dense_input, show_click)
        predict_2d = paddle.concat(x=[1 - pred, pred], axis=1)

        self.acc_metrics(pred, label_input)
        self.auc_metrics(predict_2d, label_input)
        self.mae_metrics(pred, cast_label)
        self.mse_metrics(pred, cast_label)

        # loss
        cost = paddle.nn.functional.log_loss(input=pred, label=cast_label)
