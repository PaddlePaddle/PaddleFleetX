...
def get_distributed_program():
    ...
    dist_strategy = fleet.DistributedStrategy()
    dist_strategy.semi_auto = True
    dist_strategy.auto_search = False
    fleet.init(is_collective=True, strategy=dist_strategy)
    loss, train_program, startup_program = mlp_forward(train_program,
                                                       startup_program)

    with open("./output/dp4" + "/serial_main_program.txt.%d" % (paddle.distributed.get_rank()), 'w') as f:
        f.write(str(train_program))
    with open("./output/dp4" + "/serial_startup_program.txt.%d" % (paddle.distributed.get_rank()), 'w') as f:
...
def main():
    ...
    with open("./output/mp4" + "/auto_main_program.txt.%d" % (paddle.distributed.get_rank()), 'w') as f:
        f.write(str(dist_main_prog))
    with open("./output/mp4" + "/auto_startup_program.txt.%d" % (paddle.distributed.get_rank()), 'w') as f:
    ...
    ckpt_path = [
        "./output/autosearch2/step_20/model_state_rank0.pdmodel",
        "./output/autosearch2/step_20/model_state_rank1.pdmodel",
        ]
    dist_attr_path = [
        "./output/autosearch2/step_20/dist_attr_rank0.pdattr",
        "./output/autosearch2/step_20/dist_attr_rank1.pdattr",
    ]
    ...
    for step in range(20, 50):
        # if step == 10:
        #     output_dir = "./output/mp4/" + "step_" + str(step) 
