    ...
    def __init__(self,
        ...
        if topo is None or topo.mp_info.size == 1:
            ...
            if self.fuse:
                assert self.kdim == embed_dim
                assert self.vdim == embed_dim
                self.qkv_proj = model_utils.ops.ColumnParallelLiner(
                ...
                self.q_proj = model_utils.ops.ColumnParallelLiner(
                    (embed_dim, embed_dim),
                    topo.mp_info.size,
                    gather_out=False,
                    param_attr=weight_attr1,
                    bias_attr=bias_attr)
                self.k_proj = model_utils.ops.ColumnParallelLiner(
                    (self.kdim, embed_dim),
                    topo.mp_info.size,
                    gather_out=False,
                    param_attr=weight_attr1,
                    bias_attr=bias_attr)
                self.v_proj = model_utils.ops.ColumnParallelLiner(
                    (self.vdim, embed_dim),
                    topo.mp_info.size,
                    gather_out=False,
                    param_attr=weight_attr1,
            ...
            self.out_proj = model_utils.ops.RowParallelLiner(
                (embed_dim, embed_dim),
                topo.mp_info.size,
    ...
    def __init__(self,
        ...
        if topo is None or topo.mp_info.size == 1:
            ...
            if debug:
                ...
                weight_attr1 = weight_attrs[2]
                weight_attr2 = weight_attrs[2]
            self.linear1 = model_utils.ops.ColumnParallelLiner(
                (d_model, dim_feedforward),
                topo.mp_info.size,
                gather_out=False,
                param_attr=weight_attr1,
                bias_attr=bias_attrs[2])
            self.linear2 = model_utils.ops.RowParallelLiner(
...
class GPTEmbeddings(nn.Layer):
    ...
    def __init__(self,
        ...
        if topo is None or topo.mp_info.size == 1:
            ...
            self.word_embeddings = model_utils.ops.ParallelEmbedding(
                vocab_size,
                hidden_size,
                topo,
                weight_attr=weight_attr)
    ...
    def __init__(self,
        ...
        for i in range(num_hidden_layers):
            ...
            if self.layer_per_stage is not None:
                ...
                DecoderLayer = model_utils.ops.guard('gpu:{}'.format(
        ...
        if self.pipline_mode:
            Decoder = model_utils.ops.guard('gpu:{}'.format(
