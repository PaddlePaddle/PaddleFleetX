...
class RowParallelLiner(nn.Layer):
    ...
    def forward(self, x): 
        # layers.Print(x, message="row_input_x")
        ...
        # layers.Print(x, message="row_input_split_x")
        output_parallel = self.linear(x)
        # layers.Print(output_parallel, message="output_parallel")
        output = paddle.distributed.collective._mp_allreduce(
            output_parallel,
            group=self.model_group,  # None is ok in static
            use_calc_stream=True,
            use_model_parallel=True)
        # layers.Print(output, message="output_allreduce")
