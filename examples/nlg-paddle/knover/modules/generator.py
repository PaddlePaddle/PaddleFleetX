...
class Generator(object):
    ...
    def inference(self, model, inputs, outputs):
        ...
        """
        # prepare while loop
        max_len = paddle.full(shape=[1], fill_value=self.max_dec_len)
        min_len = paddle.full(shape=[1], fill_value=self.min_dec_len)
        step_idx = paddle.full(shape=[1], fill_value=0)
        ...
        with while_op.block():
            ...
            if self.ignore_unk:
                logits = paddle.add(x=logits, y=token_penalty)

            # min dec length
            min_len_cond = paddle.less_than(x=step_idx, y=min_len)
            def min_len_penalty():
                """Plus minimum length penalty."""
                return paddle.add(x=logits, y=eos_penalty)
            def no_penalty():
                """No penalty."""
                return logits
            logits = paddle.static.nn.case([(min_len_cond, min_len_penalty)], default=no_penalty)

            # get probs
            probs = paddle.nn.functional.softmax(x=logits / self.temperature)
            ...
            if self.decoding_strategy == "beam_search":
                ...
                if self.decoding_strategy.startswith("sampling"):
                    ...
                    topk_probs, _ = paddle.topk(x=probs, k=self.topk)
                    ge_cond = paddle.cast(
                        paddle.greater_equal(x=probs, y=topk_probs[:, -1:]),
                        "float32")
                    old_probs = probs
                    probs = probs * ge_cond / paddle.sum(x=topk_probs, axis=-1, keepdim=True)
                    sampling_ids = paddle.fluid.layers.sampling_id(probs, dtype="int")
                    probs = old_probs
                elif self.decoding_strategy.startswith("topp_sampling"):
                    sorted_probs, sorted_idx = paddle.argsort(x=probs, descending=True)
                    cum_sorted_probs = paddle.fluid.layers.ops.cumsum(sorted_probs, axis=1, exclusive=True)
                    lt_cond = paddle.cast(
                        paddle.less_than(
                            x=cum_sorted_probs,
                            y=paddle.fluid.layers.fill_constant_batch_size_like(
                                cum_sorted_probs,
                    ...
                    probs = candidate_probs / paddle.sum(x=candidate_probs, axis=-1, keepdim=True)
                    sampling_ids = paddle.fluid.layers.sampling_id(probs, dtype="int")
                    sampling_ids = paddle.index_sample(sorted_idx, paddle.unsqueeze(x=sampling_ids, axis=[1]))
                    sampling_ids = paddle.squeeze(x=sampling_ids, axis=[1])
                    probs = old_probs
                else:
                    raise ValueError(self.decoding_strategy)

                sampling_scores = paddle.fluid.layers.one_hot(paddle.unsqueeze(x=sampling_ids, axis=[1]), self.vocab_size)
                sampling_scores = sampling_scores * probs - (1 - sampling_scores) * 1e3
                topk_scores, topk_indices = paddle.topk(x=sampling_scores, k=1)

            pre_len = paddle.cast(step_idx, "float32")
            paddle.increment(x=step_idx, value=1.0)
            cur_len = paddle.cast(step_idx, "float32")

            # update scores
            if self.length_average:
                accu_scores = paddle.add(
                    x=paddle.log(topk_scores), y=pre_scores * pre_len) / cur_len
            elif self.length_penalty > 0:
                pre_lp = paddle.pow(x=(5 + pre_len) / 6, y=self.length_penalty)
                cur_lp = paddle.pow(x=(5 + cur_len) / 6, y=self.length_penalty)
                accu_scores = paddle.add(
                    x=paddle.log(topk_scores), y=pre_scores * pre_lp) / cur_lp
            else:
                accu_scores = paddle.add(
                    x=paddle.log(topk_scores), y=pre_scores)
            topk_indices = paddle.fluid.layers.lod_reset(topk_indices, pre_ids)
            accu_scores = paddle.fluid.layers.lod_reset(accu_scores, pre_ids)
            selected_ids, selected_scores, parent_idx = paddle.fluid.layers.beam_search(
                pre_ids=pre_ids,
        ...
        finished_ids, finished_scores = paddle.fluid.layers.beam_search_decode(
