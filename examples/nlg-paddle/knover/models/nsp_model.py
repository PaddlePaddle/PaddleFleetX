...
class NSPModel(UnifiedTransformer):
    ...
    def get_metrics(self, inputs, outputs):
        ...
        mean_lm_loss = paddle.sum(lm_loss * need_cal) / (layers.reduce_sum(need_cal) + 1e-10)
        ...
        nsp_acc = paddle.metric.accuracy(nsp_softmax, inputs["label"])
        mean_nsp_loss = paddle.mean(nsp_loss)
    ...
    def infer(self, inputs, outputs):
        ...
        pooled_out = self._get_pooled_output(outputs["enc_out"], inputs["label_idx"])
        nsp_logits = self._get_classifier_output(pooled_out, name="next_sent")
        scores = paddle.nn.functional.softmax(x=nsp_logits)
