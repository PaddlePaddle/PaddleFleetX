...
class ResNet_ARCFACE():
    ...
    def net(self, input, label, emb_dim=512, class_dim=85164, loss_type='arcface', margin=0.5, scale=64.0):
        layers = self.layers
        ...
        conv = self.conv_bn_layer(
            input=input, num_filters=64, filter_size=3, stride=1, pad=1, act='prelu')
        ...
        bn = fluid.layers.batch_norm(input=conv, act=None)
        drop = fluid.layers.dropout(x=bn, dropout_prob=0.4, dropout_implementation='upscale_in_train')
        fc = fluid.layers.fc(
            input=drop,
            size=emb_dim,
            act=None,
        ...
        if loss_type == 'softmax':
            loss = self.fc_classify(emb, class_dim)
    ...
    def bottleneck_block(self, input, num_filters, stride):
        if self.layers < 101:
            bn1 = fluid.layers.batch_norm(input=input, act=None)
            ...
            bn0 = fluid.layers.batch_norm(input=input, act=None)
