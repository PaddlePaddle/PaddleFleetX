...
train_g.add_arg("use_recompute",          bool,   False,   "Whether to do recompute.")
train_g.add_arg("use_mix_precision",          bool,   False,   "Whether to do mix-precition training.")
...
def test(args):
    ...
    with fluid.program_guard(test_prog, test_startup):
        with fluid.unique_name.guard():
            test_pyreader, next_sent_acc, mask_lm_loss, total_loss, _ = create_model(
...
def train(args):
    ...
    with fluid.program_guard(train_program, startup_prog):
        with fluid.unique_name.guard():
            train_pyreader, next_sent_acc, mask_lm_loss, total_loss, ckpts = create_model(
                pyreader_name='train_reader', bert_config=bert_config)
            if args.use_recompute:
                dist_strategy.recompute_checkpoints = ckpts
    ...
    while steps < args.num_train_steps:
        if args.profile and steps == 10 and fleet.worker_index() == 0:
            print("begin profiler")
            profiler.start_profiler("All")
        elif args.profile and steps == 20 and fleet.worker_index() == 0:
            print("end profiler")
        ...
        try:
            ...
            if steps % skip_steps != 0:
                ...
                logging.info("epoch: %d, progress: %d/%d, step: %d, loss: %f, "
