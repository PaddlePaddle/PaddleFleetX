...
from model.transformer_encoder_paddle2 import ErnieEncoderStack, _build_linear


def _build_ln(n_in, epsilon=1e-5, name=""):
    return nn.LayerNorm(
...
class ErnieModel(object):
    def __init__(self,
        ...
        self.pre_ln = _build_ln(self._emb_size, self.config['epsilon'], 'pre_encoder')
        self.trans_lm_ln =  _build_ln(self._emb_size, self.config['epsilon'], 'mask_lm_trans')
        self.ernie_encoder = ErnieEncoderStack(self.config, 'encoder')
    ...
    def _build_model(self, emb=None):
        ...
        """
        hidden_list, _checkpoints = self.ernie_encoder(sum_emb, n_head_self_attn_mask)
        self._checkpoints = _checkpoints[1: -1]
        #self._checkpoints = [hl.name for hl in hidden_list][1: -1]
        self._enc_out = hidden_list[-1]
    ...
    def get_next_sentence_output(self, labels):
        ...
        next_sent_loss, next_sent_softmax = F.softmax_with_cross_entropy(
    ...
    def get_lm_output(self, mask_label, mask_pos):
        ...
        mask_trans_feat = fluid.layers.layer_norm(
            ...
            epsilon=self.config['epsilon'])
        """
        mask_trans_feat = self.trans_lm_ln(mask_trans_feat)
        ...
        mask_lm_loss = F.softmax_with_cross_entropy(
    ...
    def get_task_output(self, task, task_labels):
        ...
        task_loss, task_softmax = F.softmax_with_cross_entropy(
