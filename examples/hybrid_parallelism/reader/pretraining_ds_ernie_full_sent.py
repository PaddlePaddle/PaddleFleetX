...
def make_pretrain_dataset(name, gz_files, is_train, vocab, batch_size, vocab_size, max_seqlen, global_rank, world_size):
    ...
    def joint_sentences(buf, n_sub_sentence, interval_start_ends):
        ...
        for (start, end), tokens, segs in zip(interval_start_ends, tokens_of_sub_sentence, segs_of_sub_sentence):
            for chunk in buf[start: end]:
                tokens.extend(chunk[0])
                segs.extend(chunk[1])
        new_buf = []
        for t_merge, s_merge in zip(tokens_of_sub_sentence, segs_of_sub_sentence):
