...
def distributed_infer(exe, test_model, test_data_path="./data", batch_size=10):
    ...
    reader = test_model.loader.set_sample_generator(test_data, batch_size=batch_size, drop_last=True, places=place)
    ...
    with paddle.static.program_guard(main_program=dist_infer.get_dist_infer_program()):
        ...
        try:
            while True:
                ...
                batch_idx += 1
                if batch_idx % 5 == 0:
                    avg_loss = fleet.metrics.sum(loss_val) / float(fleet.worker_num())
...
def clear_metric_state(model, place):
    for metric_name in model._metrics:
        for _, state_var_tuple in model._metrics[metric_name]["state"].items():
            var = paddle.static.global_scope().find_var(state_var_tuple[0].name)
            if var is None:
                continue
            var = var.get_tensor()
