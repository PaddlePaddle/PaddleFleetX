...
def random_crop(img, size, scale=(0.08, 1.0), ratio=(3./4., 4./3.)):
    ...
    bound = min((float(img.size[0]) / img.size[1]) / (w**2), (float(img.size[1]) / img.size[0]) / (h**2))
...
def distort_color(img):
    def random_brightness(image, lower=0.5, upper=1.5):
        ...
        return ImageEnhance.Brightness(image).enhance(e)

    def random_contrast(image, lower=0.5, upper=1.5):
        e = np.random.uniform(lower, upper)
        return ImageEnhance.Contrast(image).enhance(e)

    def random_color(image, lower=0.5, upper=1.5):
        e = np.random.uniform(lower, upper)
        return ImageEnhance.Color(image).enhance(e)
