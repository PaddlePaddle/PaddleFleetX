...
近十年来，深度学习技术不断刷新视觉、自然语言、语音、搜索和推荐等领域任务的记录。这其中的原因，用一个关键词描述就是“大规模”。大规模的数据使得模型有足够的知识可以记忆，大规模参数量的模型使得模型本身有能力记忆更多的数据，大规模高性能的算力（以GPU为典型代表）使得模型的训练速度有百倍甚至千倍的提升。大规模的数据、模型和算力作为是深度学习技术的基石，在推动深度学习技术发展的同时，也带来了新的挑战。如何进行多机任务的拆分、如何配置集群训练资源、如何平衡训练速度和收敛速度、如何训练单卡无法训练的模型、弹性训练与容错等都是这个方向重点研究的课题。
