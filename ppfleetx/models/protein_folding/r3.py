...
def rigids_from_tensor4x4(m: paddle.Tensor) -> Rigids:
    ...
    assert m.shape[-2] == 4
    return Rigids(
        Rots(m[..., 0, 0], m[..., 0, 1], m[..., 0, 2], m[..., 1, 0],
             m[..., 1, 1], m[..., 1, 2], m[..., 2, 0], m[..., 2, 1],
             m[..., 2, 2]), Vecs(m[..., 0, 3], m[..., 1, 3], m[..., 2, 3]))
...
def rots_from_tensor3x3(
    ...
    assert m.shape[-1] == 3
    assert m.shape[-2] == 3
    return Rots(m[..., 0, 0], m[..., 0, 1], m[..., 0, 2], m[..., 1, 0],
                m[..., 1, 1], m[..., 1, 2], m[..., 2, 0], m[..., 2, 1],
                m[..., 2, 2])
...
def rots_from_two_vecs(e0_unnormalized: Vecs, e1_unnormalized: Vecs) -> Rots:
    ...
    e1 = Vecs(e1_unnormalized.x - c * e0.x, e1_unnormalized.y - c * e0.y,
...
def vecs_cross_vecs(v1: Vecs, v2: Vecs) -> Vecs:
    ...
    return Vecs(v1.y * v2.z - v1.z * v2.y, v1.z * v2.x - v1.x * v2.z,
                v1.x * v2.y - v1.y * v2.x)
