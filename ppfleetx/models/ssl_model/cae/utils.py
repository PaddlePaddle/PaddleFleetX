...
__all__ = [
    ...
    'VisionTransformerRegressorDecoder',
]
...
class VisionTransformerEncoder(nn.Layer):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, vocab_size=8192, embed_dim=768, depth=12,
                 ...
                 use_abs_pos_emb=True, init_std=0.02, args=None, B=4,N=122,**kwargs):
        super().__init__()
    ...
    def build_2d_sincos_position_embedding(self, embed_dim=768, temperature=10000., use_cls_token=False):
        ...
        if not use_cls_token:
            pos_embed = self.create_parameter(pos_emb.shape)
            pos_embed.set_value(pos_emb)
        else:
            pe_token = paddle.zeros([1, 1, embed_dim], dtype=paddle.float32)
            pos_emb = paddle.concat([pe_token, pos_emb], axis=1)
            #pos_embed = self.create_parameter(pos_emb.shape, default_initializer=zeros_)
            pos_embed = self.create_parameter(pos_emb.shape)
            pos_embed.set_value(pos_emb)
    ...
    def forward_features(self, x, bool_masked_pos):
        ...
        for blk in self.blocks:
            x_unmasked = blk(x_unmasked, bool_masked_pos, B=self.B, N=self.N, C=self.num_features)
...
class VisionTransformerRegressorDecoder(nn.Layer):
    def __init__(self, patch_size=16, num_classes=8192, embed_dim=768, depth=6,
                 num_heads=12, mlp_ratio=4., qkv_bias=True, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
                 drop_path_rate=0., norm_layer=None, init_values=None, num_patches=196, init_std=0.02, args=None, B=4,N=75,patch_shape=(14, 14)):
        super().__init__()
    ...
    def forward(self, x_masked, x_unmasked, pos_embed_masked, pos_embed_unmasked, bool_masked_pos):
        ...
        '''
        for ith, blk in enumerate(self.blocks):
            ...
            if not math.isfinite(x_masked.sum().item()):
                print('### check_model nan inf @ {ith}')
        ...
        if len(self.self_att_blocks) > 0:
            ...
            for blk in self.self_att_blocks:
                x_masked = blk(x_masked, B=self.B, N=self.N, C=self.num_features)
...
class Block(nn.Layer):
    ...
    def forward(self, x, bool_masked_pos=None, B=None, N=None, C=None):
        if self.gamma_1 is None:
            x = x + self.drop_path(self.attn(self.norm1(x), bool_masked_pos))
            x = x + self.drop_path(self.mlp(self.norm2(x)))
        else:
            x = x + self.drop_path(self.gamma_1 * self.attn(self.norm1(x), bool_masked_pos, B=B, N=N, C=C))
