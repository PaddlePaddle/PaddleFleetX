...
#pragma unroll
  for (int offset = 16; offset > 0; offset >>= 1) {
    T tmp_val = __shfl_down_sync(FINAL_MASK, input.v,
                                 static_cast<unsigned>(offset), 32);
    int tmp_id = __shfl_down_sync(FINAL_MASK, input.id,
                                  static_cast<unsigned>(offset), 32);
...
__global__ void KeMatrixTopPBeamTopK(const T *src, T *top_ps, int64_t *out_id,
                                     T *out_val, int vocab_size,
                                     curandState_t *state, int *count_iter,
...
__global__ void topp_sampling(T *sorted_probs, int64_t *sorted_id, T *out_val,
                              int64_t *out_id, const T *top_ps, int p_num,
...
top_p_sampling_kernel(const paddle::Tensor &x, const paddle::Tensor &top_ps) {
  ...
  auto inds_input =
      paddle::full({bs, vocab_size}, 1, paddle::DataType::INT64, x.place());
  ...
  switch (BlockSize) {
    FIXED_BLOCK_DIM(
        KeMatrixTopPBeamTopK<DataType_, TopKMaxLength, TopPBeamTopK, kBlockDim>
        <<<bs, kBlockDim, 0, cu_stream>>>(
            reinterpret_cast<DataType_ *>(
                const_cast<data_t *>(x.data<data_t>())),
            reinterpret_cast<DataType_ *>(
                const_cast<data_t *>(top_ps.data<data_t>())),
            topp_ids.data<int64_t>(),
            reinterpret_cast<DataType_ *>(topp_probs.data<data_t>()),
            vocab_size, dev_curand_states, count_iter.data<int>(),
  ...
  DataType_ *sorted_out_ptr = reinterpret_cast<DataType_ *>(
      const_cast<data_t *>(sorted_out.data<data_t>()));
  int64_t *in_id_ptr = inds_input.data<int64_t>();
  int64_t *out_id_ptr = sorted_id.data<int64_t>();

  cub::DeviceSegmentedRadixSort::SortPairsDescending(
      nullptr, temp_storage_bytes, x_ptr, sorted_out_ptr, in_id_ptr, out_id_ptr,
      vocab_size * bs, bs, segment_offsets_t_begin, segment_offsets_t_end + 1,
      0, sizeof(data_t) * 8, cu_stream);

  temp_storage_bytes = div_up(temp_storage_bytes, 256) * 256;
  int64_t temp_size = temp_storage_bytes;
  auto temp_storage =
      paddle::empty({temp_size}, paddle::DataType::UINT8, x.place());

  cub::DeviceSegmentedRadixSort::SortPairsDescending(
      temp_storage.data<uint8_t>(), temp_storage_bytes, x_ptr, sorted_out_ptr,
      in_id_ptr, out_id_ptr, vocab_size * bs, bs, segment_offsets_t_begin,
      segment_offsets_t_end + 1, 0, sizeof(data_t) * 8, cu_stream);

  switch (BlockSize) {
    FIXED_BLOCK_DIM(
        topp_sampling<DataType_, kBlockDim><<<bs, kBlockDim, 0, cu_stream>>>(
            sorted_out_ptr, out_id_ptr,
            reinterpret_cast<DataType_ *>(topp_probs.data<data_t>()),
            topp_ids.data<int64_t>(),
            reinterpret_cast<DataType_ *>(
                const_cast<data_t *>(top_ps.data<data_t>())),
            p_num, vocab_size, count_iter.data<int>(),
            count_iter_begin.data<int>()));
  default:
...
TopPSamplingInferShape(const std::vector<int64_t> &x_shape,
                       const std::vector<int64_t> &top_ps_shape) {
  std::vector<int64_t> out_ids_shape = {x_shape[0], 1};
  return {out_ids_shape};
}

std::vector<paddle::DataType>
TopPSamplingInferDtype(const paddle::DataType &x_dtype,
