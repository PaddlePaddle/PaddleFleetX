...
static int shell_popen_fork_internal(const char* real_cmd, bool do_read, int parent_end,
    ...
    // The following code is async signal safe (No memory allocation, no access to global data, etc.)
    if (child_pid != 0) {
        return child_pid;
    }
...
std::shared_ptr<FILE> shell_popen(const std::string& cmd, const std::string& mode, int* err_no) {
    ...
    return {fp,
        [child_pid, cmd, err_no](FILE * fp) {
            ...
            if (wstatus == 0 || wstatus == (128 + SIGPIPE) * 256 || (wstatus == -1
                && errno == ECHILD)) {
                //LOG(INFO) << "status[" << wstatus << "], cmd[" << cmd << "]" << ", err_no[" << *err_no << "]";
            } else {
                *err_no = -1;
                LOG(WARNING) << "status[" << wstatus << "], cmd[" << cmd << "]" << ", err_no[" << *err_no << "]";
            }
...
std::pair<std::shared_ptr<FILE>, std::shared_ptr<FILE>> shell_p2open(const std::string& cmd) {
    ...
    std::shared_ptr<int> child_life = {NULL, [child_pid, cmd](void*) {
        if (shell_verbose()) {
        ...
        PCHECK(wstatus == 0 || wstatus == (128 + SIGPIPE) * 256 || (wstatus == -1
                && errno == ECHILD)) << "status[" << wstatus << "], cmd[" << cmd << "]";
