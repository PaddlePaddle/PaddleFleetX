...
namespace ps {
	...
	//open file in 'w' or 'r'
    std::shared_ptr<FsReadChannel> AfsClient::open_r(const FsChannelConfig& config, uint32_t buffer_size) {
        int err_no = 0;
        ...
        //std::shared_ptr<FILE> fp = fs_open_read(config.path, &err_no, "");
        channel->open(fp, config);
        return channel;
    }
    std::shared_ptr<FsWriteChannel> AfsClient::open_w(const FsChannelConfig& config, uint32_t buffer_size) {
        int err_no = 0;
