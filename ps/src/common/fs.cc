...
static void fs_add_read_converter_internal(std::string& path, bool& is_pipe,
...
static void fs_add_write_converter_internal(std::string& path, bool& is_pipe,
        const std::string& converter) {
    if (converter == "") {
        return;
    }
...
static std::shared_ptr<FILE> fs_open_internal(const std::string& path, bool is_pipe,
        const std::string& mode, size_t buffer_size, int* err_no = 0) {
...
std::vector<std::string> localfs_list(const std::string& path) {
    ...
    int err_no = 0;
    pipe = shell_popen(format_string("find %s -type f -maxdepth 1", path.c_str()), "r", &err_no);
...
std::vector<std::string> hdfs_list(const std::string& path) {
    ...
    do {
    	...
    	pipe = shell_popen(format_string("%s -ls %s | ( grep ^- ; [ $? != 2 ] )",
    		hdfs_command().c_str(), path.c_str()), "r", &err_no);
    	LineFileReader reader;
    	list.clear();
...
std::string hdfs_tail(const std::string& path) {
    ...
    return shell_get_command_output(format_string("%s -text %s | tail -1 ", hdfs_command().c_str(),
            path.c_str()));
}

bool hdfs_exists(const std::string& path) {
    std::string test = shell_get_command_output(format_string("%s -test -e %s ; echo $?",
