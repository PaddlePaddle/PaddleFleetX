...
public:
    ...
    thread_queue() : _size(0), _in_use(true) {
        _head = _tail = new node_type;
        auto* beat_heart_thread = new std::thread([this](){
            while (true) {
                sleep(1);//1s
                _cv.notify_one();
            }
    ...
    value_type pop() {
        std::unique_lock<std::mutex> lck(_head_mtx);
        _cv.wait(lck, [=] { return _in_use == false || _head != get_tail(); });
